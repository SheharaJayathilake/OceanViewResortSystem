    ...
    public synchronized Connection getConnection() throws SQLException {
        ...
        for (Connection conn : connectionPool.keySet()) {
            if (Boolean.TRUE.equals(connectionPool.get(conn)) && 
                ...
                connectionPool.put(conn, false); // Mark as in-use
        ...
        if (connectionPool.size() < MAX_POOL_SIZE) {
            ...
            connectionPool.put(newConn, false); // Mark as in-use
        ...
        if (connection != null) {
            try {
                if (!connection.isClosed()) {
                    connection.setAutoCommit(true); // Reset state
                    connectionPool.put(connection, true); // Mark as available
